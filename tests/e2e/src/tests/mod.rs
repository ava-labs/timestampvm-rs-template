...
async fn e2e() {
    ...
    let genesis = {{crate_name}}::genesis::Genesis {
    ...
    let resp = cli
        .start(StartRequest {
            ...
            blockchain_specs: vec![BlockchainSpec {
                vm_name: String::from("{{crate_name}}"),
    ...
    for ep in rpc_eps.iter() {
        let resp = {{crate_name}}::client::ping(ep.as_str(), &static_url_path)
            .await
            .unwrap();
    ...
    for ep in rpc_eps.iter() {
        let resp = {{crate_name}}::client::ping(ep.as_str(), &chain_url_path)
            .await
            .unwrap();
        log::info!("ping response from {}: {:?}", ep, resp);
        assert!(resp.result.unwrap().success);
    ...
    let resp = {{crate_name}}::client::last_accepted(&ep, &chain_url_path)
        .await
        .unwrap();
    ...
    let resp = {{crate_name}}::client::get_block(&ep, &chain_url_path, &blk_id)
        .await
        .unwrap();
    ...
    let resp = {{crate_name}}::client::propose_block(&ep, &chain_url_path, vec![0, 1, 2])
        .await
        .unwrap();
    ...
    let resp = {{crate_name}}::client::propose_block(
        &ep,
        &chain_url_path,
        vec![1; {{crate_name}}::vm::PROPOSE_LIMIT_BYTES + 10],
    )
    .await
    .unwrap();
